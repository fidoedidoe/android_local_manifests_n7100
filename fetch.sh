...
fetch() {
	...
	while read line; do
		...
		echo
		echo "git -C $ANDROID_BUILD_TOP/$proj fetch https://github.com/ComicoTeam/$(path2name $proj) lineage-16.0"
		git -C $ANDROID_BUILD_TOP/$proj fetch https://github.com/ComicoTeam/$(path2name $proj) lineage-16.0
		echo ""; echo "";
